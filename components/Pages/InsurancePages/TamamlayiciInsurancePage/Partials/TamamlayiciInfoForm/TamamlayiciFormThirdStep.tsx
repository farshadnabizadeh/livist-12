...
interface TamamlayiciInfoFormThirdStepPropsType {
  handleNextStep: () => void;
  goToNextStep: () => void;
...
const TamamlayiciDurations: { label: string; value: string }[] = [
  { value: "K", label: "Kadın" },
  { value: "E", label: "Erkek" },
];

const TamamlayiciInfoFormThirdStep: FC<
  TamamlayiciInfoFormThirdStepPropsType
> = (props) => {
  ...
  useEffect(() => {
    ...
    if (areAllFormFieldsTouched() && areThereAnyErrorsInForm) {
      ...
      setValues((prevState: any) => ({ ...prevState, ...form.values }));
    } else {
      setStepsValidation((prevState) => ({
        ...prevState,
...
export default TamamlayiciInfoFormThirdStep;
