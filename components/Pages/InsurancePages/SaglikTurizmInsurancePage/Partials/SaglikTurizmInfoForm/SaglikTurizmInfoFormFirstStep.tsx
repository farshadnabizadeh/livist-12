...
import { isTCNo } from "@/utils/helpers";

interface SaglikTurizmInfoFormFirstStepPropsType {
  handleNextStep: () => void;
...
> = (props) => {
  ...
  const form = UiUseForm({
    ...
    validate: {
      ownerIdNumber: (value) =>
        value?.length < 2 ? "Bu alan zorunludur" : null,
  ...
  const handleSubmit = () => {
    const ownerIDnumber = form.values.ownerIdNumber
    const isTCNumber = isTCNo(ownerIDnumber);
    console.log('isTCNumer:', isTCNumber)

    if (!isTCNumber) {
      console.log('id inserted is not TC: ')
      const values = {
        ...form.values,
        ownerIdNumber: '',
        ownerPassportNumber: ownerIDnumber
      }

      console.log('values before setting: ', values)
    ...
    if(!values.selectedPlanCategory) return;
    ...
    <div className="flex flex-col items-center pt-4 lg:pt-[30px] px-5 md:px-[52px]">
      ...
      <form onSubmit={form.onSubmit(handleSubmit, handleError)} className="w-full">
        <div className="flex flex-col sm:grid grid-cols-1 sm:grid-cols-2 gap-4">
          <UiTextInput
            placeholder="Kimlik / Passaport Numarası"
          ...
          <PhoneInput
            ...
            containerClass="!rounded-[50px] col-span-2"
          ...
          <UiSwitch
            ...
            classNames={{
              label: "!text-white",
              root: "!col-span-2",
              track: `!border-0 ${form.values.isTicariIletiContractAccepted
                  ? "!bg-accent"
                  : "!bg-[#787880]"
                }`,
            }}
          ...
          <UiSwitch
            ...
            classNames={{
              label: "!text-white",
              root: "!col-span-2",
              track: `!border-0 ${form.values.isKVKAccepted
                  ? "!bg-accent"
                  : "!bg-[#787880]"
                }`,
            }}
...
export default SaglikTurizmInfoFormFirstStep;
