...
import { formatDate, formatDateWithSlashes } from "@/utils/helpers";
...
interface SeyahatSagilikInfoFormSecondStepPropsType {
  handleNextStep: () => void;
  goToNextStep: () => void;
...
const KapsamChoices: { label: string; value: string }[] = [
  { value: "avrupa", label: "Avrupa" },
  { value: "world", label: "Dünya" },
];
const CovidPlans: { label: string; value: string }[] = [
  { value: "dahil", label: "Dahil" },
  { value: "haric", label: "Haric" },
];

const SeyahatSagilikInfoFormSecondStep: FC<
  SeyahatSagilikInfoFormSecondStepPropsType
> = (props) => {
  ...
  const form = UiUseForm({
    ...
    validate: {
      ...
      endDate: (value: Date | null) =>
        value instanceof Date ? null : "Bitiş Tarihi Hatalı",
      ownerNationalityId: (value) => (value ? null : "Lütfen Bir Ülke Seçin"),
      kapsam: (value) => value ? null : 'Lütfen kapsam seçin',
      withCovid: (value) => value ? null : 'Lütfen Covid planı seçin'
    },
    ...
    <div className="flex flex-col items-center pt-4 lg:pt-[30px] px-5 md:px-[52px]">
      ...
      <form onSubmit={form.onSubmit(handleSubmit, handleError)} className="w-full">
        <div className="flex flex-col sm:grid grid-cols-1 sm:grid-cols-2 gap-4">
          <UiTextInput
            placeholder={`${values.ownerIdNumber ?? values.ownerPassportNumber}`}
        ...
        <UiSelect
            data={KapsamChoices}
            placeholder={t("Kapsam")}
            classNames={{ input: '!bg-white !rounded-[50px] !border-none' }}
            {...form.getInputProps("kapsam")}
          />
        <UiSelect
            data={CovidPlans}
            placeholder={t("Covid Kapsamı")}
            classNames={{ input: '!bg-white !rounded-[50px] !border-none' }}
            {...form.getInputProps("withCovid")}
          />
...
export default SeyahatSagilikInfoFormSecondStep;
