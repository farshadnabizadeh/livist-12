...
import { Group, Avatar, Text, Select, em, getBreakpointValue, Button } from "@mantine/core";
import turkeyFlag from "~/public/icons/countryFlags/turkey.svg";
import { useRouter } from "next/router";
import { UiButton, UiPopover } from "@/lib";
...
//   const { locale } = router
  ...
  return (
    <div className="inline-flex justify-end z-[1000]" ref={inputRef}>
      ...
      /> */}
      <UiPopover position={isMobile ? "bottom-end" : "bottom"} withArrow arrowOffset={200} shadow="md" opened={opened} onChange={setOpened} >
        <UiPopover.Target>
            <button className=" h-[28px] bg-[#F0CDCD] px-2 py-1 rounded-[56px] text-sm font-semibold text-white flex items-center justify-center relative" onClick={() => {setOpened(prevState => !prevState) }} type="button">
        ...
        <UiPopover.Dropdown className="absolute bg-white shadow-sm max-[768px]:!left-5 !rounded-xl" classNames={{dropdown: '!z-[1000]'}}>
