...
import seyahatSagilikPlans from "@/utils/data/seyahat-saglik-plans";
...
interface SeyahatSagilikStepsValuesType extends YabanciSagilikStepsValuesType{
...
interface SeyahatSaglikInsuranceStepsContextType {
  values: SeyahatSagilikStepsValuesType;
  setValues: React.Dispatch<
    React.SetStateAction<SeyahatSagilikStepsValuesType | undefined>
  >;
  activeStep: number;
...
const initialValues: SeyahatSagilikStepsValuesType = {
  // IDPassNumber: '',
  // birthDate: new Date(),
  // phoneNo: '',
  // insuranceDuration: 1,
...
export const SeyahatSagilikInsuranceStepsContext =
  // @ts-ignore
  React.createContext<SeyahatSaglikInsuranceStepsContextType>({
    values: initialValues,
  });

export const useSeyahatSaglikStepsContext = () => {
  return useContext(SeyahatSagilikInsuranceStepsContext);
};

type SeyahatSagilikInsuranceStepsProviderType = {
  children: ReactNode;
  branchId: string,
  productId: string,
...
export const SeyahatSaglikInsuranceStepsProvider: FC<
SeyahatSagilikInsuranceStepsProviderType
> = ({ children, branchId, paymentPlanId, paymentTypeId, productId, productTarifId, productVersionId }) => {
  const [values, setValues] =
    useState<SeyahatSagilikStepsValuesType>(initialValues);
...
console.log('this is the durationInDays', durationInDays)
console.log('timeDifference', timeDifference)
// Filter plans based on duration
const filteredPlans = seyahatSagilikPlans.filter(plan => Number(plan.Gun) >= durationInDays && plan.kapsam == values.kapsam);


console.log('here is the filteredPlans based on the day difference', filteredPlans);
  ...
  return (
    <SeyahatSagilikInsuranceStepsContext.Provider value={value}>
      {children}
    </SeyahatSagilikInsuranceStepsContext.Provider>
  );
};
